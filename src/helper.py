...
class Profit:
    ...
    def get_column_name(self, dataframe:pd.DataFrame,suffix:str)->str:
        """get column name which ends with the specified suffix
    ...
    def calculate_profit(self,position:str):
        ...
        self.entry_column = self.get_column_name(self.positions_df,'_Entry')
        self.exit_column = self.get_column_name(self.positions_df,'_Exit')
        ...
        # calculate profits
        entry_price = self.get_column_name(merged_df,'Close_Entry')
        exit_price = self.get_column_name(merged_df,'Close_Exit')
        if position == 'long':
            merged_df['profit'] = (merged_df[exit_price]-merged_df[entry_price])/merged_df[entry_price] + 1

        elif position == 'short':
            merged_df['profit'] = (merged_df[entry_price]-merged_df[exit_price])/merged_df[entry_price] + 1
        return merged_df
    
    def calculate_long_term_profit(self,profit_df:pd.DataFrame, start_year:int=0, end_year:int=9999)->float:
        ...
        return profit_df.loc[(profit_df['entry_year'].between(year_range[0], year_range[1])) & (profit_df['exit_year'].between(year_range[0], year_range[1])), 'profit'].prod()
