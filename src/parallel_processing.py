...
STOCKTREND_DEST_FILEPATH = conf.backtest_conf["preprocessing"][
    "stocktrend_dest_filepath"
]
...
class ParallelProcess(Preprocessing):
    ...
    def lazy_function(self, stock: list, prepared_df: pd.DataFrame):
        ...
        if self.strategy == "macd":
            ...
            exit_position = self.macd_positions.get_entry_exit_position(
                prepared_df[col1], prepared_df[col2]
            )
    ...
    def parallel_processing(self, prepared_df: pd.DataFrame):
        ...
        if self.strategy == "macd":
            ...
            for i in range(len(exit_position)):
                exit_position[i].rename(
                    columns=lambda x: tickers[i] + "_" + x, inplace=True
                )
